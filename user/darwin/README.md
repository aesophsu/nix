...
`default.nix` 使用顶层模块自动扫描，当前主要由 `ghostty.nix`、`profiles/` 与 `services/` 组成。
系统代理手动接管命令由 `system/darwin/system.nix` 提供（`proxy-on` / `proxy-off` / `proxy-status`）。
Shell 代理环境变量改为手动启用：`eval "$(proxy-env-on)"` / `eval "$(proxy-env-off)"`。
Python 生态镜像变量由 `user/common/core/pip.nix` 提供；通用 HTTP(S)_PROXY 不再由 `services/mihomo/` 自动注入。
