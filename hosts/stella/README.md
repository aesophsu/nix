...
Host-specific diffs only. Keep this directory small and avoid duplicating shared logic from
`system/darwin` or `user/darwin`.
...
- Do not put here: reusable macOS defaults, Homebrew policy, shared packages, shell preferences,
  services
...
If another Mac would likely need the same config, move it to `system/darwin` or `user/darwin`
