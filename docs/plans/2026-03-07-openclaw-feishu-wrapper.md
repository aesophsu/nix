...
**Goal:** Make the local `openclaw-local` flake use Feishu declaratively through a Nix wrapper
around `clawdbot-feishu`.

**Architecture:** Add a wrapper flake under `openclaw-local/plugins`, point
`programs.openclaw.customPlugins` at it, expose the upstream plugin repo as an extension tree, and
wire secret files through plugin env vars.
