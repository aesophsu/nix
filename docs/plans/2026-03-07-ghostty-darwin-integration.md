...
**Goal:** Install Ghostty via Homebrew cask and manage its macOS user configuration from this Nix
repo with a font and layout tuned for the MacBook Air M4.

**Architecture:** Keep GUI app installation in `system/darwin/apps.nix`, add a dedicated Home
Manager module under `user/darwin/` to generate `~/.config/ghostty/config`, and reuse system-managed
fonts already declared in the desktop font module. The Ghostty config will prefer a Nerd Font-backed
coding face with restrained visual settings for clarity, battery life, and smooth macOS behavior.
...
- Configure pragmatic macOS-friendly defaults: padding, scrollback, cursor, shell integration, and
