# Obsidian GUI Install Design

**Goal:** Install the Obsidian desktop application on this macOS machine declaratively through the
existing Nix-managed GUI application workflow.

**Architecture:** Reuse the current `modules/system/darwin/apps.nix` `homebrew.casks` list as the
single source of truth for rolling GUI applications. Add the `obsidian` cask there and activate
...
- A declarative entry avoids runtime drift and lets later rebuilds re-install or upgrade Obsidian.

## Verification

- Confirm `obsidian` is present in `modules/system/darwin/apps.nix`.
- Run `darwin-rebuild switch --flake .#stella`.
- Confirm Homebrew reports the Obsidian cask as installed.
