...
Run:
`rg -n "Zotero GUI Install" docs/plans/2026-03-11-zotero-gui-install-design.md docs/plans/2026-03-11-zotero-gui-install.md`
...
Run: `sed -n '1,220p' modules/system/darwin/apps.nix` Expected: existing GUI casks are declared
under `homebrew.casks`
...
Run: `git diff -- modules/system/darwin/apps.nix` Expected: only the `zotero` cask addition appears
...
Run: `rg -n '"zotero"' modules/system/darwin/apps.nix` Expected: one match in the `homebrew.casks`
list

**Step 3: Confirm Homebrew recognizes the cask**

Run: `/opt/homebrew/bin/brew list --cask zotero` Expected: exit code 0 with `zotero` listed
