...
**Goal:** Make Nix-managed `node`/`npm`/`git`/`jq` resolve before Homebrew, system, and manually
installed npm global binaries in interactive shells.

**Architecture:** Adjust the shell profile fragment that currently prepends `~/.local/npm/bin`,
`/opt/homebrew/bin`, and `/usr/local/bin` ahead of the existing PATH. Keep `NPM_CONFIG_PREFIX`, but
append the npm global bin directory to PATH instead of prepending it, so Home Manager/Nix profile
...
- Confirm `/opt/homebrew/bin` and `/usr/local/bin` are still present later in PATH if inherited by
