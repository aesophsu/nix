...
**Architecture:** Update the existing Home Manager Ghostty module in place. Because this is a
configuration-only change, verify by inspecting the generated config and evaluating the rendered
Home Manager text output rather than adding automated tests.
...
- Delete the `Symbols Nerd Font Mono` font-family line so the configuration uses only the
  Apple-native face.
...
Expected: the rendered config contains `font-family = SF Mono` and does not contain the Nerd Font
fallback line.
