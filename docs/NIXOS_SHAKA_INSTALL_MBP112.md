...
- `hosts/nixos-shaka-installer/default.nix`
- `hosts/nixos-shaka-installer/modules/install-helper.nix`
- `hosts/nixos-shaka-installer/modules/ui-tty.nix`
...
```bash
nix build .#packages.x86_64-linux.shaka-manual-installer-iso
```
...
- 推荐构建路径：从 macOS 使用 `scripts/build-nixos-iso-remote.sh` 上传到远程 Linux 构建并回传 ISO
